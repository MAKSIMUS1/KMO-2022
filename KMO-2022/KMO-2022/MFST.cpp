...
#define MFST_TRACE7		stream_out	<<std::setw(4)<<std::left<<state.lenta_position<<": "\
	...
	bool Mfst::start(std::ostream& stream_out)
	{
		...
		case NS_NORULE:
			MFST_TRACE4("---->NS_NORULE")
				stream_out << "---------------------------------------------" << std::endl;
			if(getDiagnosis(0, buf) != NULL) stream_out << getDiagnosis(0, buf) << std::endl;
			if(getDiagnosis(1, buf) != NULL) stream_out << getDiagnosis(1, buf) << std::endl;
			if(getDiagnosis(2, buf) != NULL) stream_out << getDiagnosis(2, buf) << std::endl;
	...
	void SyntaxCheck(Lex_analysis::LEX lex, Log::LOG log, std::ostream& stream_out)
	{
		MFST_TRACE_START
